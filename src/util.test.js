import { validMovesForPieceOnBoard, initCheckersBoard, kingCheckersBoard } from './util';

it('should allow non-jump moves', ()=>{
  const edgeMoves = validMovesForPieceOnBoard( initCheckersBoard, 0, 2, !'jumping' );
  expect( edgeMoves.any ).toEqual( 1 );
  expect( edgeMoves[1][3] ).toEqual( true );
  expect( edgeMoves[0][3] ).toEqual( false );

  const backMoves = validMovesForPieceOnBoard( initCheckersBoard, 1, 1 );
  expect( backMoves.any ).toEqual( 0 );

  const middleMoves = validMovesForPieceOnBoard( initCheckersBoard, 2, 2 );
...
it('should allow jump moves and not non-jump moves', ()=>{
  ...
  const nonjumpMoves = validMovesForPieceOnBoard(pieces, 2, 4);
  ...
  const jumpMoves = validMovesForPieceOnBoard(pieces, 2, 4);
...
it('should not allow non-jumps while already jumping', ()=>{
  const noMoves = validMovesForPieceOnBoard( initCheckersBoard, 0, 2, 'jumping' );
  expect( noMoves.any ).toEqual( 0 );
});


it('should allow the king to jump any direction', ()=>{
  const kingMoves = validMovesForPieceOnBoard( kingCheckersBoard, 0, 6, !'jumping' );
  expect( kingMoves.any ).toEqual( 1 );
  expect( kingMoves[2][4] ).toEqual( true );
});

it('should allow the king to move any direction', ()=>{
  const kingMoves = validMovesForPieceOnBoard( kingCheckersBoard, 5, 3, !'jumping' );
  ...
  const endKingMoves = validMovesForPieceOnBoard( kingCheckersBoard, 5, 3, 'jumping' );
...
it('should not allow non-jumps in strict mode', ()=>{
  const noMoves = validMovesForPieceOnBoard( kingCheckersBoard, 5, 1, !'jumping' );
  expect( noMoves.any ).toEqual( 0 );
});
